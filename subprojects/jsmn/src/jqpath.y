...
%code provides {
    ...
    int jqpath_lex(JQPATH_STYPE *yylval_param, yyscan_t yyscanner,
...
%{
    ...
    int jqpath_error(void *scanner, struct path_parse_context *context,
                     const char *er);
    int jqpath_lexer_init(struct path_parse_context *context, const char *in);
    void jqpath_lexer_destroy(struct path_parse_context *context);
%}
...
struct jqpath *jqpath_parse_string(const char *in) {
    ...
    if (jqpath_lexer_init(context, in) != 0) {
        path_parse_context_destroy(context);
        return NULL;
    }

    int rv = jqpath_parse(context->scanner, context);
    jqpath_lexer_destroy(context);
    if (rv != 0) {
        path_parse_context_destroy(context);
        return NULL;
    }
